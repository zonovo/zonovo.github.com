...
}},setVendorPrefixStyle:function(c,e,d){if(e.match(/^webkit/)){e=e.replace(/^webkit/,"")
}else{if(e.match(/^moz/)){e=e.replace(/^moz/,"")}else{if(e.match(/^ms/)){e=e.replace(/^ms/,"")
}else{if(e.match(/^o/)){e=e.replace(/^o/,"")}else{if(e.match("-")){var b=e.split("-");
e="";for(var a=b.length-1;a>=0;a--){e+=b[a].charAt(0).toUpperCase()+b[a].slice(1)
}}else{e=e.charAt(0).toUpperCase()+e.slice(1)}}}}}c.style["webkit"+e]=d;c.style["moz"+e]=d;
c.style["ms"+e]=d;c.style["o"+e]=d;e=e.charAt(0).toLowerCase()+e.slice(1);c.style[e]=d
},addVendorEventListener:function(b,c,d,a){if(typeof(addEventListener)=="function"){if(c.match(/^webkit/)){c=c.replace(/^webkit/,"")
}else{if(c.match(/^moz/)){c=c.replace(/^moz/,"")}else{if(c.match(/^ms/)){c=c.replace(/^ms/,"")
}else{if(c.match(/^o/)){c=c.replace(/^o/,"")}else{c=c.charAt(0).toUpperCase()+c.slice(1)
}}}}b.addEventListener("webkit"+c,d,a);b.addEventListener("moz"+c,d,a);b.addEventListener("ms"+c,d,a);
b.addEventListener("o"+c,d,a);c=c.charAt(0).toLowerCase()+c.slice(1);return b.addEventListener(c,d,a)
}},removeVendorEventListener:function(b,c,d,a){if(typeof(removeEventListener)=="function"){if(c.match(/^webkit/)){c=c.replace(/^webkit/,"")
}else{if(c.match(/^moz/)){c=c.replace(/^moz/,"")}else{if(c.match(/^ms/)){c=c.replace(/^ms/,"")
}else{if(c.match(/^o/)){c=c.replace(/^o/,"")}else{c=c.charAt(0).toUpperCase()+c.slice(1)
}}}}b.removeEventListener("webkit"+c,d,a);b.removeEventListener("moz"+c,d,a);b.removeEventListener("ms"+c,d,a);
b.removeEventListener("o"+c,d,a);c=c.charAt(0).toLowerCase()+c.slice(1);return b.removeEventListener(c,d,a)
